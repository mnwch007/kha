            ...
            <div class="mobile_link mt-4 d-block d-lg-none">
                <div class="dropdown">
                    ...
                    <ul
                        ...
                        <?php
                        $info_company_policy_type = $this->mainm->get_company_type($lang_url);
                        foreach ($info_company_policy_type as $k => $v):
                            ?>
                            <li><a class="<?= ( ((isset($type_info['id']) && $type_info['id'] == $v['id']) || !isset($type_info['id']) && $k == 0) ? 'active' : '') ?>" href="<?= base_url_lang(lang('url_lang.company_policy') . '/' . lang('url_lang.shareholder_information') . '/' . ($lang_url == 'en' ? $v['SeoURLEn'] : $v['SeoURLTh']), $lang_url) ?>"><?= $v['Name'] ?></a></li>
                            <?php
                        endforeach;
                        ?>
                    </ul>
                ...
                foreach ($detail_info as $k => $v):
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            if ($v['pdf_file'] != ""):
                                ?>
                                <span><?= FileSizeConvert(filesize(FCPATH . 'uploads/company_policy/' . $v['pdf_file'])) ?></span>
                                <a href="<?= img_path('company_policy/' . $v['pdf_file']) ?>" target="_blank"><img src="<?= assets('img/pdf.png') ?>" alt="" /></a>
                                <?php
                            endif;
                            ?>
                        </td>
