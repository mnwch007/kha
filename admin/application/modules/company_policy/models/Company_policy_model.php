...
class Company_policy_model extends MY_Model {

    var $table = "company_policy";
    var $tablelist = "company_policy_media";
    ...
    public function get_itemlist($field = '', $order = 'desc') {
        $this->db->select('company_policy.*,admin_users.full_name as updated_name,(Select type_name_en From company_type Where id=company_policy.stype)type_name_en');
        $this->db->from($this->table);
        $this->db->join('admin_users', 'admin_users.user_id = company_policy.updated_id', 'left');
        //$this->db->where('company_policy.intype', 0);
        ...
        if ($id != ''):
            ...
            if ($data) {
                if ($data['media_type'] == 0) {
                    @unlink(dirname($_SERVER["SCRIPT_FILENAME"]) . '/uploads/company_policy/' . $data['file_path']);
                } else {
                    @unlink(dirname($_SERVER["SCRIPT_FILENAME"]) . '/uploads/company_policy/' . $data['file_path']);
                }
                $this->db->delete($this->tablelist, array('id' => $id));
