...
class Financial_model extends MY_Model {
    ...
    public function get_itemlist($field = '', $order = 'desc') {
        $this->db->select('financial.*,admin_users.full_name as updated_name,(Select type_name_th From financial_type Where id=financial.stype)type_name_en');
        $this->db->from($this->table);
        $this->db->join('admin_users', 'admin_users.user_id = financial.updated_id', 'left');
        //$this->db->where('financial.intype', 0);
        ...
        if ($id != ''):
            ...
            if ($data) {
                if ($data['media_type'] == 0) {
                    @unlink(dirname($_SERVER["SCRIPT_FILENAME"]) . '/uploads/financial/' . $data['file_path']);
                } else {
                    @unlink(dirname($_SERVER["SCRIPT_FILENAME"]) . '/uploads/financial/' . $data['file_path']);
                }
                $this->db->delete($this->tablelist, array('id' => $id));
