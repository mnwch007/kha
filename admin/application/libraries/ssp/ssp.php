...
class SSP {
    ...
    static function filter($request, $columns, &$bindings, $isJoin = false) {
        ...
        if (isset($request['search']) && $request['search']['value'] != '') {
            ...
            for ($i = 0, $ien = count($request['columns']); $i < $ien; $i++) {
                ...
                if ($requestColumn['searchable'] == 'true') {
                    ...
                    $globalSearch[] = ($isJoin) ? "CONVERT(`" . $column['db'] . "` USING utf8) LIKE " . $binding : "CONVERT(`" . $column['db'] . "` USING utf8) LIKE " . $binding;
                }
            }
        }
        ...
        for ($i = 0, $ien = count($request['columns']); $i < $ien; $i++) {
            ...
            if ($requestColumn['searchable'] == 'true' && $str != '') {
                $binding = SSP::bind($bindings, '%' . $str . '%', PDO::PARAM_STR);
                $columnSearch[] = ($isJoin) ? "CONVERT(`" . $column['db'] . "` USING utf8) LIKE " . $binding : "CONVERT(`" . $column['db'] . "` USING utf8) LIKE " . $binding;
            }
        }
    ...
    static function sql_connect($sql_details) {
        try {
            $db = @new PDO(
                    "mysql:host={$sql_details['host']};dbname={$sql_details['db']};charset={$sql_details['charset']}", $sql_details['user'], $sql_details['pass'], array(PDO::ATTR_ERRMODE => PDO::ERRMODE_EXCEPTION, PDO::MYSQL_ATTR_INIT_COMMAND => 'SET NAMES utf8')
            );
